...
class GPTConfig:
    block_size: int = 1024 
    vocab_size: int = 50304 # 50527 in GPT2 (rounded up to nearest multiple of 64 for efficiency) (~50K merges, 256 vocab size, 1 special token: <|endoftext|> )
    n_layer: int = 12 
    n_head: int = 12 
    n_embd: int = 768
    dropout: float = 0.1
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        ## get positions. This is a learnable param so we use tensor
        pos = torch.arange(0, T, dtype=torch.long, device = idx.device) # same device as indices.
        tok_emb = self.transformer.wte(idx) # token embeddings of shape (B, T, n_embd)
        pos_emb = self.transformer.wpe(pos) # position embeddings of shape (T, n_embd)
        
        x = x = self.transformer.drop(tok_emb + pos_emb) # (B, T, n_embd)
        
        ## passing h through self attention heads
        for block in self.transformer.h:
            x = block(x)
        
        ## pass x through final layer norm
        x = self.transformer.ln_f(x)
        
        ## pass x through language model head to generate logits
    ...
    def configure_optimizers(self, weight_decay:float, learning_rate:float,betas:tuple(float,float), device:str):
        ...
        ## create param groups. Any parameters with weights will need to be decayed.(ndims>=2)
        ...
        print(f"Number of decayed tensors: {len(decay_params)} with num decay params: {num_decay_params}, Number of non-decayed tensors: {len(nodecay_params)} with num no decay params: {num_nodecay_params}")
        
        # create an adamW optimizer and use fused version if available
